...
- Specs longues (Sanitize 2 Mo + **compacteur déterministe** > 38 Ko) · personnalisation visuelle par app ·
  ...
  **enrichissement du Spec Normalizer** (extraction déterministe depuis la prose : enums → V1.10 ; **bornes/unités/défauts numériques + requis-optionnel → V1.11**, Architect V5.5) ·
  ...
  **mode amélioration intelligent** : propositions déterministes (structurelles + compagnes par domaine), aperçu du diff avant application, historique des versions V1→Vn · **correctif (Orchestrateur V6.15 + V6.16)** : en mode amélioration, l'orchestrateur transmet enfin `improve_mode` au Spec Linter → le prompt incrémental court (« Ajoute la ressource X… ») n'est plus rejeté par la règle de longueur minimale (le contexte complet reste le ZIP existant analysé par le ZIP Analyzer). V6.15 ajoutait le flag dans le nœud « Build Spec Linter Payload », mais le nœud HTTP « Agent Spec Linter » avait un corps figé (`{task_id, userRequest}`) qui l'omettait → V6.16 corrige aussi ce corps (`{task_id, improve_mode, userRequest}`), sinon le bypass ne se déclenchait jamais ·
  ...
  **correctif Secrets faux positif (Backend V6.20)** : les mots de passe de *fixtures* de la suite de tests sont assemblés à l'exécution (`['Passw','0rd!123'].join('')`) au lieu d'apparaître en littéral `password: '…'` — supprime une détection « Hardcoded Password » (règle heuristique du scanner de secrets) qui stoppait le pipeline une fois la porte SAST franchie. Le scanner reste strict sur les vrais secrets (clés AWS/Stripe/OpenAI, PEM, JWT). ·
  **QA rendu strictement advisory (Agent QA V5.7)** : le gate QA (piloté par LLM) ne peut plus **bloquer** le pipeline (REWORK). La sécurité est tranchée par les gates **déterministes et autoritaires** (SAST/SCA/Secrets/DAST) qui, eux, STOPpent sur un vrai problème ; un « high sécurité » émis par le LLM — non déterministe, sur du code déjà vérifié par les scanners — devient un **WARN visible**, jamais un FAIL. Aligne le comportement sur le principe du projet (*LLM = compréhension, déterministe = sécurité*) et supprime les REWORK intempestifs dus à la variance du jugement LLM. ·
  ...
  **garde-fou SAST du mode amélioration (Backend V6.22)** : en amélioration, le LLM réécrit `server.js` ; désormais sa version n'est acceptée que si elle ne contient AUCUN motif SAST-critique (mêmes règles que l'Agent SAST : eval, injection SQL par concaténation, Math.random sur un secret, exec, XSS…). Sinon → repli sur la version **déterministe** `base_server_js`, qui implémente déjà l'amélioration de façon sûre. Le LLM ne peut plus introduire de faille critique dans le code livré (respect strict du principe « LLM = compréhension, déterministe = sécurité »). ·
  **déploiement clef-en-main (Backend V6.21, axe 3)** : chaque app générée embarque un **Dockerfile multi-stage** (1re étape compile better-sqlite3 natif, 2e étape image runtime allégée sans outils de build), un **docker-compose** (volume de persistance de la base SQLite via `DB_PATH=/data`), un **snippet nginx** (`client_max_body_size 8m` → règle le 413 des uploads ; en-têtes `X-Forwarded-*` pour l'IP réelle à l'audit ; réglages SSE pour le temps réel derrière proxy) et un **DEPLOY.md** — déploiement en `docker compose up -d --build`. ·
  **Normalizer enrichi V1.11** : un passage déterministe extrait de la prose, en plus des énumérations, les **bornes numériques** (« entre 0 et 5000 », « sur 5 », « au plus 120 », « positif »), les **unités** (€, FCFA, $, kg, %, …), les **valeurs par défaut** (« par défaut 0 ») et le **caractère requis/optionnel** (« obligatoire » / « facultatif ») — attachés au modèle de données comme `{ type:'number', min, max, unit, default }` et `required_fields`. Alimente directement les champs riches (V6.16), optionnels (V6.18) et la validation serveur **sans typage manuel du modèle**. Prudence : recherche insensible aux accents, fenêtre courte autour du nom de champ, **unités de temps exclues** (anti-faux-positif type « livré entre 2 et 5 jours »), et jamais de conversion d'un champ texte en nombre.
...
| Étape | IA ? | Rôle |
|---|---|---|
| **Spec Normalizer** | ✅ LLM (codestral) + **enrichissement déterministe** | **Cœur** : lit une spec en langage naturel / PDF / DOCX (prose, tables, ambiguë) et en extrait un **modèle structuré** (ressources, champs, types, routes, règles, acteurs, permissions, deferred). Irremplaçable : un parseur déterministe ne comprend pas du texte humain libre. **V1.11** : un passage déterministe détecte en plus, dans la prose, les **énumérations** (« statut : en attente, expédiée, livrée » → `{type:'enum', values:[...]}`), les **bornes/unités/défauts numériques** (« prix en euros, entre 0 et 5000 » → `{type:'number', min:0, max:5000, unit:'€'}`, « note sur 5 », « par défaut 0 ») et le **caractère requis/optionnel** (« obligatoire » / « facultatif ») — attachés au modèle → formulaires riches (listes déroulantes, bornes de saisie, unités, champs optionnels) + validation serveur, **sans typage manuel**. Prudence : noms de champs candidats, 2–12 valeurs pour les enums, recherche insensible aux accents, unités de temps exclues, jamais de faux positif sur du texte libre. |
