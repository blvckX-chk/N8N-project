...
- Specs longues (Sanitize 2 Mo + **compacteur déterministe** > 38 Ko) · personnalisation visuelle par app ·
  ...
  **mode amélioration intelligent** : propositions déterministes (structurelles + compagnes par domaine), aperçu du diff avant application, historique des versions V1→Vn · **correctif (Orchestrateur V6.15)** : en mode amélioration, l'orchestrateur transmet enfin `improve_mode` au Spec Linter → le prompt incrémental court (« Ajoute la ressource X… ») n'est plus rejeté par la règle de longueur minimale (le contexte complet reste le ZIP existant analysé par le ZIP Analyzer) ·
