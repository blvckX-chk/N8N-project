...
- ✅ **PWA installable** — **livré** (Frontend V6.22) : `manifest.json` + service worker qui ne met en cache QUE la coquille statique (index/app.js/style/manifest/icône/login), jamais l'API ni les données métier → aucune fuite hors-ligne. Icône SVG déterministe, enregistrement du SW depuis app.js (CSP 'self' préservée). App installable sur mobile/desktop.
